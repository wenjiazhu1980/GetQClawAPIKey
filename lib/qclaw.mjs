...
const QCLAW_APP_STORE_PATH_WIN = path.join(
...
function getAppStorePath() {
  if (process.env.QCLAW_APP_STORE_PATH) return process.env.QCLAW_APP_STORE_PATH;
  if (process.platform === 'win32') return QCLAW_APP_STORE_PATH_WIN;
  return QCLAW_APP_STORE_PATH;
}
...
function decryptDPAPI(base64Blob) {
  const psScript =
    `Add-Type -AssemblyName System.Security;` +
    `$e=[Convert]::FromBase64String('${base64Blob}');` +
...
function decryptStoredValue(storedValue, password) {
  if (typeof storedValue === 'string') {
    // On Windows, Electron safeStorage stores a base64-encoded DPAPI blob.
    // Try DPAPI first; fall back to plaintext if it fails.
    if (process.platform === 'win32') {
      try {
        return decryptDPAPI(storedValue);
      } catch {
  ...
  // macOS Chromium v10 encrypted format
  const pass = password !== undefined ? password : getKeychainPassword();
  return decryptChromiumV10(storedValue.cipherText, pass).trim();
}
...
function readQClawAuthSession() {
  const store = readQClawStore();
  const password = process.platform === 'darwin' ? getKeychainPassword() : undefined;
